...
/* harmony import */ var _modules_getWeather__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./modules/getWeather */ "./src/js/modules/getWeather.js");
/* harmony import */ var _modules_time__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./modules/time */ "./src/js/modules/time.js");
/* harmony import */ var _modules_liveSearch__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./modules/liveSearch */ "./src/js/modules/liveSearch.js");



new _modules_getWeather__WEBPACK_IMPORTED_MODULE_0__["default"]().init();
new _modules_time__WEBPACK_IMPORTED_MODULE_1__["default"]().init();
new _modules_liveSearch__WEBPACK_IMPORTED_MODULE_2__["default"]().init(); // window.addEventListener('load', () => {
// });

/***/ }),

/***/ "./src/js/modules/getWeather.js":
/*!**************************************!*\
  !*** ./src/js/modules/getWeather.js ***!
  \**************************************/
...
function () {
  ...
  _createClass(GetWeather, [{
    key: "getCityCoord",
    value: function getCityCoord() {
      var posApi, get, data, status;
      return regeneratorRuntime.async(function getCityCoord$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              _context.prev = 0;
              posApi = "https://api.openweathermap.org/data/2.5/weather?q=".concat(this.city, "&appid=43ba6481c3bd842c9f1c3a6cdf5c6957&units=").concat(this.units);
              _context.next = 4;
              return regeneratorRuntime.awrap(fetch(posApi));

            case 4:
              get = _context.sent;
              _context.next = 7;
              return regeneratorRuntime.awrap(get.json());
            ...
            case 12:
              ...
              throw new Error('Please type the correct location name!');

            case 14:
              this.lat = data.coord.lat;
              this.lng = data.coord.lon;
              this.apiLocation = "".concat(data.name, ", ").concat(data.sys.country.toUpperCase());
              _context.next = 23;
              break;
    ...
    value: function getGeoLocation() {
      ...
      this.gpsBtn.addEventListener('click', function () {
        _this.input.value = '';
        console.log('gps');
        ...
        if (navigator.geolocation) {
          navigator.geolocation.getCurrentPosition(function (pos) {
            ...
            var gpsRes = "https://api.openweathermap.org/data/2.5//weather?lat=".concat(_this.lat, "&lon=").concat(_this.lng, "&appid=43ba6481c3bd842c9f1c3a6cdf5c6957&units=").concat(_this.units);
            fetch(gpsRes).then(function (res) {
              return res.json();
            }).then(function (data) {
              _this.apiLocation = "".concat(data.name, ", ").concat(data.sys.country.toUpperCase());
    ...
    value: function getWeather() {
      ...
      var todayDescription, todayDegree, locationName, todayIcon, todayHumidity, todayWind, cards, weeklyWeatherApi, get, data, weatherIcon;
      return regeneratorRuntime.async(function getWeather$(_context2) {
        while (1) {
          switch (_context2.prev = _context2.next) {
            case 0:
              // Today forecast
              todayDescription = document.querySelector('.today__description'), todayDegree = document.querySelector('.today__degree'), locationName = document.querySelector('.location-title'), todayIcon = document.querySelector('.today__icon'), todayHumidity = document.querySelector('.today__himidity'), todayWind = document.querySelector('.today__wind'); //Cards - next 6 days forcast

              cards = document.querySelectorAll('.daily__card');
              weeklyWeatherApi = "https://api.openweathermap.org/data/2.5/onecall?lat=".concat(this.lat, "&lon=").concat(this.lng, "&cnt=7&appid=6e4b78723374bfb2085e80fe383222a3&units=").concat(this.units, "&exclude=hourly");
              _context2.next = 5;
              return regeneratorRuntime.awrap(fetch(weeklyWeatherApi));
            ...
            case 8:
              data = _context2.sent;
              console.log(data);
              weatherIcon = "http://openweathermap.org/img/wn//".concat(data.current.weather[0].icon, "@2x.png"); // Set DOM elements from the API for today forcast
              ...
              todayDegree.textContent = "\n            ".concat(Math.round(data.current.temp), " \n            ").concat(this.units === 'metric' ? '\xB0C' : '\xB0F', " ");
              todayDescription.textContent = data.current.weather[0].description;
              todayIcon.innerHTML = "<img src=\"".concat(weatherIcon, "\">");
              todayHumidity.innerHTML = "<i class=\"fas fa-tint\"></i> ".concat(data.current.humidity, " %");
              todayWind.innerHTML = "\n            <i class=\"fas fa-wind\"></i>\n            ".concat(this.units === 'metric' ? 'km/h' : 'mph', " "); // Set DOM elements from the API for daily forcast

              cards.forEach(function (card, i) {
                var date = new Date(data.daily[i + 1].dt * 1000);
                card.children[0].textContent = _this2.weekdays[date.getDay()].slice(0, 3).toUpperCase();
                card.children[1].src = "http://openweathermap.org/img/wn//".concat(data.daily[i + 1].weather[0].icon, "@2x.png");
                card.children[2].textContent = data.daily[i + 1].weather[0].description;
                card.children[3].textContent = "".concat(Math.round(data.daily[i + 1].temp.day)).concat(_this2.units === 'metric' ? '\xB0C' : '\xB0F', " ");
                card.children[4].innerHTML = "<i class=\"fas fa-tint\" ></i> ".concat(data.daily[i + 1].humidity, " % ");
                card.children[5].innerHTML = "<i class=\"fas fa-wind\" ></i> ".concat(Math.round(data.daily[i + 1].wind_speed), "\n            ").concat(_this2.units === 'metric' ? 'km/h' : 'mph', " ");
              });
    ...
    key: "onSearchBtn",
    value: function onSearchBtn(btn) {
      var _this3 = this;

      btn.addEventListener('click', function () {
        _this3.city = _this3.input.value;
    ...
    value: function onEnter() {
      ...
      this.input.addEventListener("keyup", function (e) {
        ...
        if (e.keyCode === 13) {
          ...
          _this4.searchBtn.click();
        }
      });
    }
  }, {
    key: "degreeUnits",
    value: function degreeUnits() {
      var _this5 = this;

      var checkbox = document.getElementById('unitSwitch');
      checkbox.addEventListener('click', function () {
        if (checkbox.checked) {
          _this5.units = 'imperial';
          console.log('checked');
        } else {
          _this5.units = 'metric';
        ...
        console.log('Degree method : ', _this5.lat, _this5.lng);
...
/* harmony import */ var _getWeather__WEBPACK_IMPORTED_MODULE_16__ = __webpack_require__(/*! ./getWeather */ "./src/js/modules/getWeather.js");
...
function (_GetWeather) {
  _inherits(LiveSearch, _GetWeather);
  ...
  function LiveSearch() {
    ...
    _this.resultList = document.querySelector('.location__result-list');
    return _this;
  } // getting city list 
  ...
  _createClass(LiveSearch, [{
    key: "cities",
    value: function cities(city) {
      var _this2 = this;

      var findCityApi, res, data;
      return regeneratorRuntime.async(function cities$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              findCityApi = "https://andruxnet-world-cities-v1.p.rapidapi.com/?query=".concat(city, "&searchby=city");
              _context.next = 3;
              return regeneratorRuntime.awrap(fetch(findCityApi, {
                "headers": {
            ...
            case 6:
              ...
              data.forEach(function (item) {
                ...
                if (item.city.search(new RegExp("^" + city, "i")) !== -1) {
                  listItem.textContent = "".concat(item.city);

                  _this2.resultList.appendChild(listItem);
                ...
                var children = _this2.resultList.children;
                var set = new Set();
                ...
                listItem.addEventListener('click', function () {
                  _this2.resultList.style.display = "none";
                  _this2.city = listItem.textContent;
    ...
    value: function init() {
      ...
      this.input.addEventListener('keyup', function () {
        _this3.resultList.innerHTML = '';

        var val = _this3.input.value.trim();

        if (val.length >= 3) {
          _this3.resultList.style.display = 'block';

          _this3.cities(val);
        } else {
          _this3.resultList.style.display = 'none';
        }
      });
    }
  }]);
  ...
  return LiveSearch;
}(_getWeather__WEBPACK_IMPORTED_MODULE_16__["default"]);
...
function () {
  function Time() {
    ...
    this.date = document.querySelector('.time__date'); // this.timeInfo = new Date();
  }
  ...
  _createClass(Time, [{
    ...
    value: function updateTime() {
      ...
      if (hr < 10) {
        this.hours.textContent = "0".concat(hr);
      } else {
        this.hours.textContent = hr;
      }

      if (min < 10) {
        this.minutes.textContent = "0".concat(min);
      } else {
        this.minutes.textContent = min;
      }

      if (sec < 10) {
        this.seconds.textContent = "0".concat(sec);
      } else {
      ...
      var day = dateInfo.getDate(); // store date

      var currentDate = weekday[dateInfo.getDay()] + ", " + month[dateInfo.getMonth()] + " " + day;
